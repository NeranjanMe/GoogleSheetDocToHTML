function ConvertGoogleDocsToCleanHtml() {
  ...
  var sheet = ss.getSheets()[0]; // assuming URLs are in the first sheet
  var range = sheet.getRange(1, 1, sheet.getLastRow(), 1); // assuming URLs are in the first column, starting from row 1
  var urls = range.getValues();

  for (var i = 0; i < urls.length; i++) {
    var url = urls[i][0];
    var id = url.match(/[-\w]{25,}/); // extract doc id from url
    if (id) {
      ConvertGoogleDocToCleanHtml(id[0]);
    } else {
      Logger.log('Invalid URL at row ' + (i+1));
    }
  }
}

function ConvertGoogleDocToCleanHtml(id) {
  ...
  saveHtmlToDrive(html, images, id);
}

function saveHtmlToDrive(html, images, id) {
  var folderId = 'Your_Drive_ID'; // replace with your folder's id
  var folder = DriveApp.getFolderById(folderId);

  var name = DocumentApp.openById(id).getName()+".html";
  var file = folder.createFile(name, html, MimeType.HTML);

  for (var j=0; j<images.length; j++) {
    folder.createFile(images[j].blob);
  }
}
